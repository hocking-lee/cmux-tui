...
cmux-tui --port /dev/ttyUSB0 --frame 8E1               # 非 8N1 的模块
cmux-tui --port /dev/ttyUSB0 --append crlf             # 改追加的结束符
...
帧格式默认 `8N1`，用 `--frame` 改，写法就是串口工具通用的那个记号：数据位
`5`–`8`、校验 `N`/`E`/`O`（大小写不敏感）、停止位 `1`/`2`，例如 `7E1`、`8O2`。
状态栏显示的是实际生效的值。流控固定为 None——CMUX 用硬件流控会干扰帧同步，
没有开放这个开关。
...
| `Ctrl+G` | 显示快捷键一览（任意键关闭） |
| `Ctrl+L` / `Ctrl+S` / `Ctrl+Q` | 清当前通道 / 日志开关 / 退出 |

不用背这张表——界面输入栏右侧常驻 `Ctrl+G 快捷键` 提示，按一下就能看到全部键位。
帮助用 `Ctrl+G` 而不是惯例的 `F1`，因为 `F1`/`F2` 已经用于切换通道槽位。

模式切换用 `Ctrl+T` 而不是更顺手的 `Ctrl+H`：终端里 `Ctrl+H` 发送的是 `0x08`，
会被解析成退格键，两者冲突。

ASCII 模式发送时默认追加 `\r`，用 `--append` 改成 `none`/`cr`/`lf`/`crlf`——
输入栏会显示当前设置（如 `ASCII +\r\n`）。HEX 模式下不追加任何字节，
输入 `41 54 0D` 或 `41540D` 均可。

有些模块的 CMUX 实现逐字节成帧（实测 Quectel EC800M），一个响应会拆成几十个
1 字节的 UIH 帧。工具会把 `--merge-window` 毫秒内连续到达的同向数据并成一条显示
（默认 5ms，设 0 关闭）。帧边界信息不丢——`Ctrl+R` 的原始帧视图仍然逐帧列出。
