...
fn input_line(state: &AppState) -> Line<'static> {
    ...
    let mut spans = vec![
        ...
        // ASCII 模式才会追加结束符，HEX 模式下显示它会误导
        Span::styled(
            if state.input.mode == crate::app::input::InputMode::Ascii {
                format!("+{} ", state.cfg.line_ending.label())
            } else {
                String::new()
            },
...
mod tests {
    ...
    #[test]
    fn input_bar_shows_the_line_ending_in_ascii_mode() {
        use crate::app::input::LineEnding;
        let mut s = ready_state();
        s.cfg.line_ending = LineEnding::CrLf;
        let out = render(&s, 90, 20);
        assert!(out.contains("+\\r\\n"), "应显示追加的结束符，实际:\n{out}");
    }

    #[test]
    fn hex_mode_hides_the_line_ending() {
        use crate::app::input::LineEnding;
        let mut s = ready_state();
        s.cfg.line_ending = LineEnding::CrLf;
        s.input.toggle_mode(); // 切到 HEX
        let out = render(&s, 90, 20);
        assert!(
            !out.contains("+\\r\\n"),
            "HEX 模式不追加任何字节，显示结束符会误导，实际:\n{out}"
        );
    }

    #[test]
    fn input_bar_advertises_the_help_key() {
        let out = render(&ready_state(), 90, 20);
        assert!(out.contains("Ctrl+G"), "快捷键得能被发现，实际:\n{out}");
    }
