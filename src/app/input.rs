...
/// ASCII 模式下自动追加在输入末尾的结束符。
///
/// AT 命令按规范用 `\r` 结束，但有些模块或自定义协议要 `\r\n` 甚至不要结束符，
/// 所以做成可配置。HEX 模式不受影响——那里用户完全掌控每个字节。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum LineEnding {
    /// 不追加任何字节。
    None,
...
impl std::str::FromStr for LineEnding {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_ascii_lowercase().as_str() {
            ...
            other => Err(format!("追加符号必须是 none/cr/lf/crlf，收到 {other:?}")),
        }
    }
}
...
/// 把输入框文本解析成待发送字节。
///
/// `ending` 只在 ASCII 模式生效；HEX 模式下用户完全掌控每个字节。
pub fn parse_input(text: &str, mode: InputMode, ending: LineEnding) -> Result<Vec<u8>, String> {
    match mode {
        InputMode::Ascii => {
            if text.is_empty() {
...
mod tests {
    ...
    fn ascii_mode_sends_literal_text() {
        assert_eq!(parse_input("AT", InputMode::Ascii, LineEnding::None).unwrap(), b"AT");
    }

    #[test]
    fn ascii_mode_appends_cr_when_asked() {
        assert_eq!(parse_input("AT", InputMode::Ascii, LineEnding::Cr).unwrap(), b"AT\r");
    }

    #[test]
    fn hex_mode_accepts_spaced_bytes() {
        assert_eq!(
            parse_input("41 54 0D", InputMode::Hex, LineEnding::None).unwrap(),
            vec![0x41, 0x54, 0x0D]
        );
    }
    ...
    fn hex_mode_accepts_run_together_bytes() {
        assert_eq!(
            parse_input("41540D", InputMode::Hex, LineEnding::None).unwrap(),
            vec![0x41, 0x54, 0x0D]
        );
    }
    ...
    fn hex_mode_ignores_append_cr() {
        ...
        assert_eq!(parse_input("41", InputMode::Hex, LineEnding::Cr).unwrap(), vec![0x41]);
    }

    #[test]
    fn hex_mode_rejects_odd_digit_count() {
        let err = parse_input("415", InputMode::Hex, LineEnding::None).unwrap_err();
        assert!(err.contains("偶数"), "实际: {err}");
    }

    #[test]
    fn hex_mode_rejects_non_hex_characters() {
        let err = parse_input("4G", InputMode::Hex, LineEnding::None).unwrap_err();
        assert!(err.contains("非法"), "实际: {err}");
    }

    #[test]
    fn empty_input_is_rejected_in_both_modes() {
        assert!(parse_input("", InputMode::Ascii, LineEnding::Cr).is_err());
        assert!(parse_input("   ", InputMode::Hex, LineEnding::None).is_err());
    }

    #[test]
    fn line_ending_appends_the_right_bytes() {
        assert_eq!(parse_input("AT", InputMode::Ascii, LineEnding::Lf).unwrap(), b"AT\n");
        assert_eq!(
            parse_input("AT", InputMode::Ascii, LineEnding::CrLf).unwrap(),
            b"AT\r\n"
        );
    }

    #[test]
    fn line_ending_parses_from_cli_values() {
        use std::str::FromStr;
        assert_eq!(LineEnding::from_str("none").unwrap(), LineEnding::None);
        assert_eq!(LineEnding::from_str("cr").unwrap(), LineEnding::Cr);
        assert_eq!(LineEnding::from_str("lf").unwrap(), LineEnding::Lf);
    ...
    #[test]
    fn line_ending_rejects_unknown_value_readably() {
        use std::str::FromStr;
        let e = LineEnding::from_str("rn").unwrap_err();
        assert!(e.contains("none/cr/lf/crlf"), "实际: {e}");
        assert!(e.contains("rn"), "应回显用户输入，实际: {e}");
    }

    #[test]
    fn default_line_ending_is_cr() {
        assert_eq!(LineEnding::default(), LineEnding::Cr);
        assert_eq!(LineEnding::default().bytes(), b"\r");
    }
