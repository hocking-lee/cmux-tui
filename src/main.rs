...
use cmux_tui::app::input::LineEnding;
...
use cmux_tui::serial::params::FrameFormat;
...
struct Cli {
    ...
    /// 连续数据的合并窗口（毫秒），0 关闭合并
    ///
    /// 有些模块逐字节成帧，一个响应会拆成几十个 1 字节的 UIH 帧。
    /// 窗口内连续到达的同向数据会并成一条显示，原始帧视图仍逐帧记录。
    #[arg(long, default_value_t = 5)]
    merge_window: u64,

    /// ASCII 模式发送时追加的结束符：none / cr / lf / crlf
...
fn main() -> anyhow::Result<()> {
    ...
    let mut stream = SerialStream::open(&port, cli.baud, cli.frame, at_cfg.timeout)
    ...
    let app_cfg = AppConfig {
        line_ending: cli.append,
        merge_window: Duration::from_millis(cli.merge_window),
        ..AppConfig::default()
    };
    let mut rt = Runtime::new(app_cfg, SessionConfig::default(), tx_req);
    rt.state.port_desc = format!("{port} {} {}", cli.baud, cli.frame);
    rt.state.link = link;
